...
static void print_begintoend(GList *list) {
   ...
   for (; iterator.unequal(&iterator, &end);iterator.next(&iterator)) {
        msg = iterator.data(&iterator);
        print_Msg_t(msg);
   }
}
...
static void print_rbegintorend(GList *list) {
   ...
   Msg_t *msg = NULL;
   printf("GList: empty %d  size %d\n", list->empty(list), list->size(list));
   for (iterator = list->rbegin(list); iterator.unequal(&iterator, &end); iterator.next(&iterator)) {
        msg = iterator.data(&iterator);
        print_Msg_t(msg);
   }
}
...
static void test_list_nodes(GList *list) {
    gpointer val = list->front(list);
    Msg_t *msg =  (Msg_t *)val, *msg1 = NULL;
    int size;
    print_Msg_t(msg);
...
static void test_list_alloc_1() {
    ...
    list  = g_list_alloc(sizeof(Msg_t));
    list1 = g_list_alloc(sizeof(Msg_t));
